...
@click.option(
    "--package-name",
    prompt="Python package name (e.g. 'mixture-of-depths-jax')",
...
def main(
    ...
    replacements = [
        ...
        ("mixture-of-depths-jax", package_actual_name),
        ("mixture_of_depths_jax", package_dir_name),
    ]
    if dry_run:
    ...
    # Rename 'mixture_of_depths_jax' directory to `package_dir_name`.
    if not dry_run:
        (REPO_BASE / "mixture_of_depths_jax").replace(REPO_BASE / package_dir_name)
    else:
        print(f"Renaming 'mixture_of_depths_jax' directory to '{package_dir_name}'")
